...
detect_nvidia_adjust() {
  ...
  if lspci -k | grep -A 2 -E "(VGA|3D)" | grep -iq nvidia; then
    ...
    sed -i '/env = LIBVA_DRIVER_NAME,nvidia/s/^#//' config/hypr/CONF FILES/configs/ENVariables.conf
    sed -i '/env = __GLX_VENDOR_LIBRARY_NAME,nvidia/s/^#//' config/hypr/CONF FILES/configs/ENVariables.conf
    sed -i '/env = NVD_BACKEND,direct/s/^#//' config/hypr/CONF FILES/configs/ENVariables.conf
    sed -i '/env = GSK_RENDERER,ngl/s/^#//' config/hypr/CONF FILES/configs/ENVariables.conf
    sed -i 's/^\([[:space:]]*no_hardware_cursors[[:space:]]*=[[:space:]]*\)2/\1 1/' config/hypr/CONF FILES/configs/SystemSettings.conf
  fi
}
...
detect_vm_adjust() {
  ...
  if hostnamectl | grep -q 'Chassis: vm'; then
    ...
    sed -i 's/^\([[:space:]]*no_hardware_cursors[[:space:]]*=[[:space:]]*\)2/\1 1/' config/hypr/CONF FILES/configs/SystemSettings.conf
    sed -i '/env = WLR_RENDERER_ALLOW_SOFTWARE,1/s/^#//' config/hypr/CONF FILES/configs/ENVariables.conf
    sed -i '/monitor = Virtual-1, 1920x1080@60,auto,1/s/^#//' config/hypr/CONF FILES/monitors.conf
  fi
}
...
detect_nixos_adjust() {
  ...
  if hostnamectl | grep -q 'Operating System: NixOS'; then
    ...
    local DISABLE_SA="config/hypr/CONF FILES/configs/Startup_Apps.disable"
