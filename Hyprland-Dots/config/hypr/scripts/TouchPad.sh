...
laptops_conf="$HOME/.config/hypr/CONF FILES/UserConfigs/Laptops.conf"
...
if [[ -z "$touchpad_device" ]]; then
    notify-send -u low -i "$notif" " Touchpad" " Device name not set (check CONF FILES/UserConfigs/Laptops.conf)"
    exit 1
fi
