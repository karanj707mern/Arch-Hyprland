    ...
    "action1")  
      if [ -d "$KooL_Dots_DIR" ]; then
        ...
        kitty -e bash -c '
          ...
          git pull &&
          ./copy.sh &&
          notify-send -u critical -i "$1/ja.png" "Update Completed:" "Kindly log out and relogin to take effect"
        ' "$KooL_Dots_DIR" "$iDIR"
        ...
        kitty -e bash -c '
          git clone --depth=1 https://github.com/JaKooLit/Hyprland-Dots.git "$0" &&
          cd "$0" &&
          chmod +x copy.sh &&
          ./copy.sh &&
          notify-send -u critical -i "$1/ja.png" "Update Completed:" "Kindly log out and relogin to take effect"
